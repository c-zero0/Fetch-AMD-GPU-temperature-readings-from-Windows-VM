...
// Menu action control
void MenuControl(IADLXPerformanceMonitoringServicesPtr perfMonitoringServices, IADLXGPUPtr oneGPU, int direct_monitor);
        ...
        if (ADLX_SUCCEEDED(res))
        {
            ...
            if (ADLX_SUCCEEDED(res))
            {
                ...
                if (ADLX_SUCCEEDED(res))
                {
                    ...
                    MenuControl(perfMonitoringService, oneGPU, 1);
                }
                else
...
void ShowGPUHotspotTemperature(IADLXGPUMetricsSupportPtr gpuMetricsSupport, IADLXGPUMetricsPtr gpuMetrics)
{
    ...
    if (ADLX_SUCCEEDED(res))
    {
        ...
        if (supported)
        {
            ...
            if (ADLX_SUCCEEDED(res))
                std::cout << "The GPU hotspot temperature is: " << hotspotTemperature <<" C" << std::endl;
        }
    }
}
...
void ShowCurrentGPUMetrics(IADLXPerformanceMonitoringServicesPtr perfMonitoringServices, IADLXGPUPtr oneGPU)
{
    ...
    // output constantly the informations
